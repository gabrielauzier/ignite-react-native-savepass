...
export function RegisterLoginData() {
  ...
  async function handleRegister(formData: Partial<FormData>) {
    ...
    try {
      const currentLoginData = await AsyncStorage.getItem(dataKey);
      const currentLoginDataFormatted = currentLoginData
        ? JSON.parse(currentLoginData)
        : [];

      const newLoginDataFormatted = [
        ...currentLoginDataFormatted,
        newLoginData,
      ];

      await AsyncStorage.setItem(
        dataKey,
        JSON.stringify(newLoginDataFormatted)
      );
      navigate("Home");
    } catch (error) {
      console.log(error);
